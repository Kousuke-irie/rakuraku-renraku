<script setup>
// 監視ダッシュボードの「全社」タブ（P4-4 / monitoring.md §6）
//
// ★このファイルの中身は DashboardView.vue から**そのまま移設**したもの。
//   個人タブ（P4-8）を足すにあたって、画面の器（タブ）と中身を分けただけで、
//   集計・配色・パネル構成は変えていない。
//
...
//   使っていない段階（例：四次・五次面接）は含まれない。
/** 標準フロー外の段階には印を付ける。設定を変えた後に取り残された学生がいるということ */
const selectionLabel = (row) => (row.isEnabled ? row.label : `${row.label}（フロー対象外）`)

const selectionChart = computed(() => {
  const rows = data.value?.selectionBreakdown ?? []

  return {
    labels: rows.map(selectionLabel),
    datasets: [
      {
...
<template>
  <div class="overview">
    ...
    <div class="grid">
      <ChartPanel
        title="選考ステータス別 学生数"
        note="全学生。選考フローの設定で有効な段階を、その並び順で表示します"
