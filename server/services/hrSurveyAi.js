// 人事FBアンケートの自由記述をAIでまとめる（S-12）。
//
// 人事が読みたいのは「20件の原文」ではなく「選考を終えた学生が、うちの対応の
// 何を良いと感じ、何に引っかかったか」。原文は根拠として下に並べ、上に要約を置く。
//
// ★外部（Gemini）へ送るのは自由記述の本文と★だけ。
//   学生名・大学名・ルームID・担当者名・回答日時は送らない。そもそも
//   listHrSurveyComments が返さない（hrSurveys.js の匿名化がここでも効いている）。
//
// 構造は services/interviewSurveyAi.js（S-11）に合わせている。キャッシュの持ち方、
// 多重呼び出しの束ね方、失敗時の状態は同じ考え方で読めるようにするため。

import { AI_SUMMARY_STATUS, HR_SURVEY_SCOPE_ALL } from '../../shared/constants.js';
import { GEMINI_API_KEY, GEMINI_MODEL } from '../config/gemini.js';
import { listHrSurveyComments } from './hrSurveys.js';

const GEMINI_API_BASE_URL = 'https://generativelanguage.googleapis.com/v1beta/models';
export const HR_SURVEY_AI_TIMEOUT_MS = 8_000;

/** 箇条書きの上限。多いと「で、結局どこを直すのか」が読み取れなくなる */
export const HR_SURVEY_AI_POINT_LIMIT = 4;
...
const OUTPUT_SCHEMA = Object.freeze({
  ...
  properties: {
    ...
    positives: {
      type: 'ARRAY',
      maxItems: HR_SURVEY_AI_POINT_LIMIT,
...
function buildPrompt(comments) {
  return [
    'あなたは新卒採用の選考体験を改善する分析アシスタントです。',
    '与えられたアンケートの自由記述だけを根拠に、日本語でまとめてください。',
    'これは面接そのものではなく、採用担当者との「やり取り」（連絡の速さ・説明の',
    '分かりやすさ・対応の丁寧さ）についての回答です。',
    `positives と concerns はそれぞれ最大${HR_SURVEY_AI_POINT_LIMIT}件です。`,
    '複数の回答に共通して現れる内容を優先し、1件だけの意見を全体の傾向として書かないでください。',
    '各項目は40文字以内の体言止めにしてください。',
    '該当する内容が無ければ空配列にしてください。無理に項目を作らないでください。',
    '入力に無い事実、学生個人を特定する記述、担当者への人格評価を書かないでください。',
...
function validateOutput(value, generatedAt) {
  ...
  const toPoints = (input) => {
    ...
    return input
      ...
      .filter(Boolean)
      .slice(0, HR_SURVEY_AI_POINT_LIMIT)
...
async function requestGemini({ comments, apiKey, model, fetchImpl, now }) {
  const response = await fetchImpl(
    `${GEMINI_API_BASE_URL}/${encodeURIComponent(model)}:generateContent`,
    {
      ...
      signal: globalThis.AbortSignal.timeout(HR_SURVEY_AI_TIMEOUT_MS),
    },
  );
 ...
 *
 * ★スコープの匿名性判定は listHrSurveyComments に任せる。
 *   回答が少ない担当者はコメントが空で返るので、要約も作らない。
 *   ここで独自に判定を書かないこと（閾値が2箇所に散る）。
 *
 * @param {string} scopeId 'all' / 担当者ID / 'unknown'
 */
export function generateHrSurveySummary(
  db,
  {
    scopeId = HR_SURVEY_SCOPE_ALL,
    force = false,
  ...
  const { isSuppressed, comments } = listHrSurveyComments(db, scopeId);
  ...
  // ★本文と総合★だけを送る。選考結果（内定／辞退）すら送らない
  //   （担当者と組み合わせると回答者を絞り込める）
  const payload = comments.map((comment) => ({
    rating: comment.avgOverall,
    body: comment.body,
  }));
  ...
  const promise = (async () => {
    try {
      ...
      console.warn(
        `server: hr survey AI summary failed` +
...
export function clearHrSurveyAiCache() {
  summaryCache.clear();
