...
`/dashboard`。閲覧は `role='admin'` 限定（未決事項 Q-4「admin ロールの権限差をデモで見せるか」への回答を兼ねる）。
ナビレールには「全学生」の下に追加する。
...
- **色**：**全バー同一色 `#3B7FC4`**。10段階を10色にしない
  - 段階の違いは「バーの位置」が既に示している。色に仕事はない
  - 10色を作ると必ず色覚多様性のチェックを落ちる（8色が上限、9色目は既存色と区別できない）
- **辞退**：`declined` は進行段階ではなく離脱なので、**罫線で区切って最下段**に置き、`#D03B3B` で塗る
...
| ダッシュボードを人事全員に開放する | 担当者別の遵守率は評価情報。admin 限定にする |
