...
- **色は2色だけ**：選考に残っている `#3B7FC4` ／ 離脱 `#D03B3B`
  - 4区分を4色にすると「確定(緑)」と「離脱(赤)」が隣り合い、P型・D型色覚で
    区別できない（実測 CVD ΔE 5.2）。**紫を挟む案も通常視 ΔE 11.2 で落ちた**
  - 色は「選考に残っているか／離れたか」だけを担わせ、4区分は**表の「区分」列**で示す
  - 2色構成は全ペアで PASS（CVD ΔE 21.2 / 通常視 28.9）
...
| 選考ステータスの4区分を4色で塗り分ける | 「確定(緑)」と「離脱(赤)」が隣接し、P型・D型色覚で区別できない（実測 CVD ΔE 5.2）。紫を挟む案も通常視 ΔE 11.2 で落ちた |
